...
module loopback_node_agent (
    ...
    logic [10:0] memory [0:DEPTH-1];
    logic [7:0]  wptr, rptr;
    logic [8:0]  count;
    logic is_writing, is_reading;

    assign is_writing = rx_tvalid && rx_tready[0];
    assign is_reading = tx_tvalid && tx_tready[0];

    assign rx_tready[0] = (count < DEPTH);
    assign rx_tready[1] = 1'b0;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (is_writing) begin
                memory[wptr] <= {rx_tlast, rx_tid, rx_tdata};
                wptr <= wptr + 1'b1;
            end
            if (is_reading) begin
                rptr <= rptr + 1'b1;
            ...
            if (is_writing && !is_reading)
                count <= count + 1'b1;
            else if (!is_writing && is_reading)
                count <= count - 1'b1;
        end
    end

    assign tx_tvalid = (count > 0);
    assign tx_tlast  = memory[rptr][10];
    assign tx_tid    = memory[rptr][9:8];
    assign tx_tdata  = memory[rptr][7:0];

    assign tx_tdest  = 4'b0000;

    (* mark_debug = "true" *) logic tx_tvalid_dbg;
    (* mark_debug = "true" *) logic tx_tready_dbg;
    (* mark_debug = "true" *) logic tx_tlast_dbg;
    (* mark_debug = "true" *) logic [7:0] tx_tdata_dbg;
    (* mark_debug = "true" *) logic [3:0] count_dbg;
...
module packet_arbiter #(
    ...
    logic single_xfer;
    assign single_xfer = |(grant & valid) && ready;
