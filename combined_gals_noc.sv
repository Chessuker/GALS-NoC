...
module arty_gals_noc_wrapper (
    ...
    logic [7:0] t00_tx_data;  logic [3:0] t00_tx_dest;  logic t00_tx_valid;  logic [1:0] t00_tx_ready;
    logic [7:0] t00_rx_data;  logic t00_rx_valid;  logic [1:0] t00_rx_ready;

    logic [7:0] t01_tx_data;  logic [3:0] t01_tx_dest;  logic t01_tx_valid;  logic [1:0] t01_tx_ready;
    logic [7:0] t01_rx_data;  logic t01_rx_valid;  logic [1:0] t01_rx_ready;

    logic [7:0] t10_tx_data;  logic [3:0] t10_tx_dest;  logic t10_tx_valid;  logic [1:0] t10_tx_ready;
    logic [7:0] t10_rx_data;  logic t10_rx_valid;  logic [1:0] t10_rx_ready;

    logic [7:0] t11_tx_data;  logic [3:0] t11_tx_dest;  logic t11_tx_valid;  logic [1:0] t11_tx_ready;
    logic [7:0] t11_rx_data;  logic t11_rx_valid;  logic [1:0] t11_rx_ready;
    ...
    gals_noc_top uut_noc_top (
        ...
        .h00_tx_tdata(t00_tx_data), .h00_tx_tdest(t00_tx_dest), .h00_tx_tvalid(t00_tx_valid), .h00_tx_tready(t00_tx_ready),
        .h00_tx_tid(2'b01), .h00_tx_tlast(1'b1),
        .h00_rx_tdata(t00_rx_data), .h00_rx_tvalid(t00_rx_valid), .h00_rx_tready(t00_rx_ready),
        .h00_rx_tdest(), .h00_rx_tid(), .h00_rx_tlast(),

        .h01_tx_tdata(t01_tx_data), .h01_tx_tdest(t01_tx_dest), .h01_tx_tvalid(t01_tx_valid), .h01_tx_tready(t01_tx_ready),
        .h01_tx_tid(2'b01), .h01_tx_tlast(1'b1),
        .h01_rx_tdata(t01_rx_data), .h01_rx_tvalid(t01_rx_valid), .h01_rx_tready(t01_rx_ready),
        .h01_rx_tdest(), .h01_rx_tid(), .h01_rx_tlast(),

        .h10_tx_tdata(t10_tx_data), .h10_tx_tdest(t10_tx_dest), .h10_tx_tvalid(t10_tx_valid), .h10_tx_tready(t10_tx_ready),
        .h10_tx_tid(2'b01), .h10_tx_tlast(1'b1),
        .h10_rx_tdata(t10_rx_data), .h10_rx_tvalid(t10_rx_valid), .h10_rx_tready(t10_rx_ready),
        .h10_rx_tdest(), .h10_rx_tid(), .h10_rx_tlast(),

        .h11_tx_tdata(t11_tx_data), .h11_tx_tdest(t11_tx_dest), .h11_tx_tvalid(t11_tx_valid), .h11_tx_tready(t11_tx_ready),
        .h11_tx_tid(2'b01), .h11_tx_tlast(1'b1),
        .h11_rx_tdata(t11_rx_data), .h11_rx_tvalid(t11_rx_valid), .h11_rx_tready(t11_rx_ready),
        .h11_rx_tdest(), .h11_rx_tid(), .h11_rx_tlast()
    );

    logic pass_g1, pass_g2, any_fail;

    noc_stress_tester mega_tester (
        .clk_h00(clk_h00), .clk_h01(clk_h01), .clk_h10(clk_h10), .clk_h11(clk_h11),
        .rst_n(global_rst_n),

        .t00_tx_data(t00_tx_data), .t00_tx_dest(t00_tx_dest), .t00_tx_valid(t00_tx_valid), .t00_tx_ready(t00_tx_ready),
        .t00_rx_data(t00_rx_data), .t00_rx_valid(t00_rx_valid), .t00_rx_ready(t00_rx_ready),

        .t01_tx_data(t01_tx_data), .t01_tx_dest(t01_tx_dest), .t01_tx_valid(t01_tx_valid), .t01_tx_ready(t01_tx_ready),
        .t01_rx_data(t01_rx_data), .t01_rx_valid(t01_rx_valid), .t01_rx_ready(t01_rx_ready),

        .t10_tx_data(t10_tx_data), .t10_tx_dest(t10_tx_dest), .t10_tx_valid(t10_tx_valid), .t10_tx_ready(t10_tx_ready),
        .t10_rx_data(t10_rx_data), .t10_rx_valid(t10_rx_valid), .t10_rx_ready(t10_rx_ready),

        .t11_tx_data(t11_tx_data), .t11_tx_dest(t11_tx_dest), .t11_tx_valid(t11_tx_valid), .t11_tx_ready(t11_tx_ready),
        .t11_rx_data(t11_rx_data), .t11_rx_valid(t11_rx_valid), .t11_rx_ready(t11_rx_ready),
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (rx_tvalid) begin
                ...
                if (rx_tdata != ~PAYLOAD) fail <= 1'b1;
            end

            if (rx_count == 10 && !fail) begin
                pass <= 1'b1;
            end
        end
    end
